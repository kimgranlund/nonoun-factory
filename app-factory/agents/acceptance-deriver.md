...
- **Resist the memorizing stub (calibration-learned, 2026-06-26).** A bar with HARDCODED literal inputs is satisfiable by a fixture-memorizing stub that never does the real work — e.g. for a key-value store, `assert load('k') == 'v'` is passed by `def load(k): return 'v' if k=='k' else None`, which never stores. The entailment-critic refuses these. Author the bar to defeat the stub: exercise **multiple distinct inputs** with **non-literal, seeded-deterministic values** (`rnd = random.Random(<fixed seed>)` → reproducible but not memorizable), drive **all state changes before all reads** (defeats a single-slot stub), use **identity checks** where the prose demands them (`is None`, not `== None`), and where the prose implies update semantics add an **overwrite/re-read** step. See `evals/calibration/exemplars.json` (`R1`/`R2` are the certified shape; `E1`/`E6` are the refused hardcoded-literal anti-pattern).
