...
> **The one write path is gated, not a bypass.** The ▶ Run loop button POSTs `/api/project/<name>/loop`, which the server fulfils by invoking the controller (`${CLAUDE_PLUGIN_ROOT}/bin/app-loop.py run`) — armed fail-closed, signals minted by the independent `app-validator`, every step ledgered. Authoring mutations (`/app-new`, `/app-spec`) stay in the commands. The UI drives the factory **through** its gates; it never writes state directly.
