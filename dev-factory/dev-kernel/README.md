...
| **Roster + skills** | a 19-agent roster (`agents/`) across 8 skills — **7 core lattice** skills + **1 meta** skill + a 7-agent spec review council (see *Skill layering*) |
...
The **7 core lattice skills** operate the invariant machine over the on-disk grid: `lattice-management` · `ticket-orchestration` · `cell-engine` · `verification` · `regeneration` · `autonomy-governance` · **`spec-author`** — the factory's *intake boundary*, where a human intent / PRD / notes becomes a typed spec, **managed as a SKILL-format artifact** (a spec is a mini-skill) across its whole life (author → review → refine → update). It carries the **`spec-council`** review council (a `spec-council` orchestrator + 6 read-only lens-critics) and gives the `spec-architect` agent its home. One **meta skill** — `kit-authoring` — extends the *family*: it builds **family kits** (the stateless plugins in `../dev-kit-*`, never the kernel), and the kernel's own `check-kit-conform.py` mechanically enforces that boundary (a kit that needs a kernel edit has leaked). It ships here because the kit-conform contract it authors against *is* a kernel gate.
...
- *"author a spec from this PRD/notes"* · *"review this spec — is it ready?"* · *"the spec was wrong — update it"* — **spec-author** (the intake boundary + the spec council)
