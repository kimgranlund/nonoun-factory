...
**Every app needs a runnable entry — seed the shell.** When the bound kit declares a single-file shell authoring (a browser-app kit like `dev-kit-app`), the decomposition MUST include a **`capability.system.shell`** cell — the bootable `index.html` at the product root that imports + mounts the built modules — depending on every capability module. A decomposition that seeds the modules but omits the shell is **incomplete**: the modules exist but nothing boots, and the factory's "drained" reads as "done" when the app can't actually run. This isn't discretionary — `api.app_completeness` mechanizes it: a shell-declaring instance with built capability modules and no validated shell reports `bootable: false` and `factory_state` reads **`incomplete`**, not drained. Seed the shell with the slice, validated by the kit's render-coherence gate (a real WebGL frame / a mounted DOM), not a presence stub.
