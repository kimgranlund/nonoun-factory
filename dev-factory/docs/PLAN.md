...
**Status:** **delivered through the Run phase (Fly partial).** The system built here ships as `dev-kernel` (0.2.11), `dev-kit-corpus` (0.3.0), `dev-kit-app` (0.3.1), and the `dev-server` runtime. **Source of truth for what's delivered:** the per-component CHANGELOGs + `dev-server/evals/` + `dev-server/RUNBOOK.md`. **Companion design:** `docs/specs/dev-factory-spec/`. This document is the build sequence + load-bearing decisions + remaining frontier — kept as the forward view, not a restatement of the spec.
