...
- **The loop's caps are enforced in code, AND the arming gap is now closed (I-9, v0.5.0)** — `/harness-run` runs the `harness-builder` orchestrator under hard caps, and the **enforcement** is the wired `gate-budget`, not the agent's counting. The loop's two-step preamble is itself mechanized: **step 0a `run-budget.py mark`** sets a **loop-active marker**, and from that instant `gate-budget` denies *every* write **until a budget is armed** — so **step 0b `run-budget.py start`** is no longer skippable into silence: a marked-but-un-budgeted loop **fails closed**, not unbounded. Once the budget exists, the gate denies every write past `now ≥ deadline` (wall-clock) or the ledger-counted iterations/cells cap — **the loop physically cannot write past its budget** (`evals/global-bound/` proves both the cap enforcement *and* the arming-gap closure with no model agent). The marker is precisely what distinguishes the autonomous loop from a human: manual edits and attended `/harness-advance` never set it, so they stay free; `/harness-status` shows the arming-gap state explicitly; and the marker file is deny-on-write to workers (a worker can't clear its own marker to escape). The residual shrank from "forget step 0" to "skip the entire preamble". Per-cell, `ledger.py no-progress` → block → `gate-budget` deny (`evals/stop-gate/`). Attended by design; autonomy is **earned, not declared**.
...
- **v0.5 is the kernel + the operating roster + the wired loop + the bounded autonomous loop with the arming gap closed + the structural council, validated, selftested, behaviorally evidenced, and dogfood-red-teamed.** CI replays the engine loop (`evals/first-slice-walkthrough/`), proves the per-cell stop-gate (`evals/stop-gate/`), proves the **global run-budget enforced in code with no model agent AND the v0.5.0 arming-gap closure** (`evals/global-bound/`: a loop marked active but un-budgeted is denied every write, while manual editing stays free — the I-9 fix for the one residual the four red-teams kept naming), and proves the gates catch the planted defect classes (`evals/calibration/`). The council-as-separate-plugin, the discriminated-union schema (evidence-less validation made unrepresentable), a token/dollar budget meter, streaming observability, trust-trajectory automation, verifier adapters, and family **kits** are on the [ROADMAP](ROADMAP.md).
