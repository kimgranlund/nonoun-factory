...
0a. **mark the loop active** — `run-budget.py mark --label "harness-run …"`. The mechanical first act: it flips the loop into a state where the wired `gate-budget` **denies every write until a budget is armed** — so if you skip 0b, the loop fails *closed*, not unbounded (the I-9 arming-gap fix). Manual edits and `/harness-advance` never set this marker, so they stay free.
0b. **start the run budget** — `run-budget.py start --max-iterations N --max-cells M --wall-clock-s S`. **This arms the ceiling** (and keeps the marker). It persists the **global** budget; from here `gate-budget` denies *every* write once the run is exhausted (wall-clock deadline, or ledger-counted iterations/cells past the cap) — once armed, the loop cannot write past its budget. (A cap-less budget is refused; between 0a and 0b the gate already denies, so forgetting 0b can't run free.)
...
5. **stop** — `run-budget.py stop` at the end (clears the budget **and** the marker; a fresh run is marked + armed deliberately).
