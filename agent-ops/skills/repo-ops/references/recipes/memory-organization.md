...
> **The premise.** Memory primitives only compound if they have _one_ obvious home each. The most common organizational failure is not absence — it's split-brain: `docs/ops/adrs/` AND `docs/decisions/`, both half-populated, both authoritative-looking. The agent loads both, gets contradictions, and learns nothing. This recipe makes the layout decisive.

## The four memory primitives

| Primitive | What it captures | Storage | Lifecycle |
| --- | --- | --- | --- |
| **ADR** | A specific architectural commitment | `docs/ops/adrs/NNNN-*.md` | Proposed → Accepted → (Superseded) |
| **Postmortem** | An incident retrospective | `docs/ops/postmortems/YYYY-MM-DD-*.md` | Drafted → Resolved (mostly immutable) |
| **Runbook** | A repeatable operational procedure | `docs/ops/runbooks/<verb-noun>.md` | Living; revised in place |
...
| --- | --- | --- |
| **Architecture overview** | `docs/ARCHITECTURE.md` (or `docs/ops/architecture/`) | Describes current state, not decision history |
...
| --- | --- |
| "We decided to use X instead of Y" (architectural) | `docs/ops/adrs/` |
| "We picked library X over Y" (small, reversible) | `docs/decisions/` (optional) |
| "On April 12, the checkout broke because..." | `docs/ops/postmortems/` |
| "To roll out a new region, run these 8 steps" | `docs/ops/runbooks/` |
...
## Should I have `docs/ops/adrs/` AND `docs/decisions/`?
...
**Some teams: yes.** When ADRs feel too heavy for routine choices ("we picked Vitest over Jest"), a lighter `docs/decisions/` folder reduces activation energy. Trade-off: now you have two places to look. Document the split in `docs/ops/adrs/README.md`:

```markdown
# Decision logs

This repo splits decisions into two folders by weight:
- `docs/ops/adrs/` — architectural commitments (MADR format, full context).
...
| The decision itself, after it's made | `docs/ops/adrs/NNNN-*.md` |
...
| --- | --- | --- |
| `docs/ops/adrs/` | `NNNN-kebab-case-title.md` (4-digit, sequential) | Numbered for sequence; immutable after Accepted |
| `docs/ops/postmortems/` | `YYYY-MM-DD-incident-name.md` | Sorts chronologically; date is the unique identifier |
| `docs/decisions/` | `YYYY-MM-DD-decision-name.md` | Sorts chronologically; small enough that numbering is overkill |
| `docs/ops/runbooks/` | `<verb>-<noun>.md` | No date — runbooks are living |
| `docs/ops/architecture/` | `<topic>.md` (kebab-case) | Topical, not temporal |
| `docs/drafts/` | freeform | Not yet committed |
| `docs/ops/archive/` | `original-name-YYYY-MM-DD.md` | Provenance preserved for moved-orphans |
...
- **Architecture Decision Records:** `docs/ops/adrs/` (index: `docs/ops/adrs/README.md`)
- **Decision log (lighter-weight):** `docs/decisions/` (only if your team uses both)
- **Post-mortems:** `docs/ops/postmortems/` (index: `docs/ops/postmortems/README.md`)
- **Runbooks:** `docs/ops/runbooks/` (index: `docs/ops/runbooks/README.md`)
...
```markdown
## Memory primitives
- **Before architectural decisions**, read `docs/ops/adrs/` newest-first.
- **When debugging production issues**, search `docs/ops/postmortems/`.
- **When running operational procedures**, look in `docs/ops/runbooks/`
...
- **`docs/ops/adrs/` and `docs/decisions/` both half-populated, no documented split.** Pick one or document the split.
- **Postmortems in `docs/traces/`.** "Traces" reads as observability traces (OpenTelemetry / Jaeger). Use `docs/ops/postmortems/` or `docs/incidents/`.
