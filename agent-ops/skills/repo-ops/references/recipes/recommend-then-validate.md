...
```yaml
# docs/ops/fix-plan-2026-04-27.yaml (transient; consumed then discarded)
strictness: normal
findings_addressed:
...
fixes:
  ...
  - kind: archive
    ...
    move_to: docs/ops/archive/old-plan-2026-04-27.md
...
| `delete` | **Always vetoed.** Move to `docs/ops/archive/` instead. Hard rule, no overrides. |
...
```text
1. Main agent runs the audit, builds fix plan, writes docs/ops/fix-plan-*.yaml
2. Main agent spawns validator subagent: "Read docs/ops/fix-plan-*.yaml;
