...
Thresholds are **defined by the reliability-dial resolver** (`../guidance/reliability-dial.md` § "The resolver") per strictness — the single source of truth. `audit-skills.mjs` reads them from `.claude/repo-ops.toml` rather than hardcoding; the values above are the resolver's `normal`-level defaults.
