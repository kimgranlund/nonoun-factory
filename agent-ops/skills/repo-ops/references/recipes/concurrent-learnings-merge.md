...
| --- | --- | --- |
| ADRs (`docs/ops/adrs/<file>.md`) | One file per decision | **Only the next-number race** (two PRs both want `0042-`) |
| Postmortems (`docs/ops/postmortems/YYYY-MM-DD-*.md`) | Date-prefixed, one per incident | Same-day collision → suffix `-2` |
| Audit-history ledger (`docs/ops/audit-history/<date>.json`) | Append-only, dated | None — can't conflict |
...
Replace `docs/ops/adrs/0042-postgres-choice.md` with **`docs/ops/adrs/2026-04-27-1342-postgres-choice.md`**.
...
```text
docs/ops/changesets/2026-04-27-vitest-replaces-jest.md
```
...
1. Reads all `docs/ops/changesets/*.md` files in chronological order.
...
5. **Write a new superseding ADR** (`docs/ops/adrs/<timestamp>-resolve-X-vs-Y.md`) with:
...
`.claude/repo-ops.toml`:

```toml
[repo-ops.merge]
adr_naming = "timestamp"          # "timestamp" | "sequential" | "hybrid"
agents_md_changesets = true       # require docs/ops/changesets/ for AGENTS.md edits
...
- **Letting changesets accumulate without consolidation.** Once `docs/ops/changesets/` has >5 files unconsolidated, the audit emits a finding. Consolidate at least weekly.
