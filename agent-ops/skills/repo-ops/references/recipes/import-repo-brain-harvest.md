...
> **The premise.** Some other repo ran `harvest repo-ops --export` and produced a portable bundle. **`import repo-ops harvest`** ingests that bundle into _this_ repo's `docs/ops/` tree, then syncs the new content into the agent's memory at `~/.claude/projects/<this-repo>/memory/` so the next session catches everything immediately.
...
| --- | --- | --- |
| `bundle/adrs/NNNN-*.md` | `docs/ops/adrs/` | Renumber on collision (next free `NNNN`); preserve original number in body as `Originally: 0007 (source-repo)` |
| `bundle/postmortems/YYYY-MM-DD-*.md` | `docs/ops/postmortems/` | Date prefix is unique enough; on exact-match append `-imported` to filename |
| `bundle/postmortems/_TEMPLATE.md` | `docs/ops/postmortems/_TEMPLATE.md` | If absent in target, install. If present, diff and prompt — don't auto-overwrite |
| `bundle/runbooks-portable/*.md` | `docs/ops/runbooks/` | Same diff-and-prompt rule |
| `bundle/conventions-portable/*.md` | `docs/conventions/` | Same — never auto-overwrite |
| `bundle/findings-schema.json` | (use to validate `docs/ops/findings/INDEX.md`) | If target has no `INDEX.md`, install one with the imported schema |
| `bundle/audit-history-schema.json` | (use to validate `docs/ops/audit-history/`) | First imported audit lands in the same shape as the source |
| `bundle/config.toml.example` | `.claude/repo-ops.toml` | If target has no config, install with imported defaults; otherwise diff-and-prompt |
...
3. **Detect target's brain layout.**
   - `docs/ops/` exists? Use it.
   - Only legacy `docs/{adrs,postmortems,...}/` exists? Recommend running base `repo-ops` to migrate to `docs/ops/` first; abort import unless user forces.
...
7. **Update local INDEX files.**
   - `docs/ops/adrs/README.md` (if present): refresh the index to include imported ADRs with a `(imported from <source-repo>)` annotation
   - `docs/ops/postmortems/INDEX.md`: same
   - `docs/ops/findings/INDEX.md`: keep target's findings, but add an `Imported from <source>` section if the source had `## Graduations` content worth preserving
...
12. **Report.** Print a summary:
    ...
    Audit run:
      ✓ Trip-wires green; ledger entry written to docs/ops/audit-history/2026-04-29-import-source.json
    ```
...
- [ ] Bundle size > 5 MB
- [ ] Target has uncommitted changes in `docs/ops/` (require clean tree to make rollback safe)
...
The import never touches anything outside `docs/ops/` + `docs/conventions/` + `~/.claude/projects/<repo>/memory/` + `.claude/settings.json`. To roll back:

```bash
git checkout -- docs/ops/ docs/conventions/ .claude/settings.json
