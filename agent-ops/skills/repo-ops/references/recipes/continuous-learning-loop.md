...
2. **Architectural decisions get captured at decision time** as ADRs in `docs/ops/adrs/`, not retroactively reconstructed from PR descriptions six months later.
...
- name: Architectural-impact ADR check
  run: |
    ...
    new_adrs=$(git diff --name-only $base...HEAD -- 'docs/ops/adrs/*.md' | grep -c '^' || echo 0)
...
**The trip-wire.** `docs/ops/adrs/` should grow over time at a rate consistent with the repo's architectural-change rate. A 5-year-old repo with 2 ADRs is suspicious — either the team's not capturing decisions or they're capturing them somewhere else (PR descriptions, Slack, code comments). The audit flags repos with `<1 ADR per year of repo age` for `>1 year` repos.

### Flow 3 — Postmortem-on-incident

**The mechanism.** Within 1 week of a SEV-1 or SEV-2 incident, a blameless post-mortem lands in `docs/ops/postmortems/YYYY-MM-DD-name.md`. See `../doc-types/postmortem-pattern.md` for the template.
...
**The trip-wire.** If the repo has had production incidents (heuristic: search `git log` for `revert`, `hotfix`, `incident`, `outage`, `rollback`) but `docs/ops/postmortems/` is empty, the audit flags it. False positives are fine — the prompt to write a post-mortem is the value.
...
- **Before architectural changes**, read `docs/ops/adrs/` newest-first. If
...
| ADR-on-architectural-change | `docs/ops/adrs/` exists and contains ≥1 ADR per year of repo age (rough heuristic) | Medium |
| ADR PR template | `.github/pull_request_template.md` includes architectural-impact checkbox | Low |
| Postmortem-on-incident | If `git log` has incident-shaped commits, `docs/ops/postmortems/` is non-empty | Medium |
...
- **ADRs in PR descriptions** — they don't compound; nobody can find them. Move into `docs/ops/adrs/`.
- **Post-mortems in private docs (Notion, Confluence)** — unreachable from the agent. Either move to `docs/ops/postmortems/` or link aggressively from AGENTS.md.
