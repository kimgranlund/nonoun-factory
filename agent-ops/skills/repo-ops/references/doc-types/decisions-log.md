...
> **The canonical "decision log" is the index of all ADRs in `docs/ops/adrs/`. It is _not_ a separate folder.** This is the load-bearing nuance most teams get wrong.
...
So when a repo has `docs/ops/adrs/0001-…md` … `0042-…md` and a `docs/ops/adrs/README.md` listing them, **that is the decision log**. There is no second folder needed.
...
| --- | --- | --- |
| **Canonical (single-folder)** | `docs/ops/adrs/` containing all decisions; `docs/ops/adrs/README.md` is the log index | Default. Fits 90% of teams. |
| **Split (two-folder)** | `docs/ops/adrs/` for big architecture decisions; `docs/ops/decisions/` for lighter day-to-day picks (tooling swaps, library choices, formatting conventions) | Larger orgs where ADR review is heavyweight and would gate routine choices |

The split is **a team convention, not industry standard**. Some teams find it useful: ADRs gate architecture, `docs/ops/decisions/` lets engineers move on a tooling swap without scheduling an architecture review. Other teams find it confusing — two homes invites drift over which decision goes where.

The audit's recommendation order:

1. If only `docs/ops/adrs/` exists → recommend keeping it canonical; ensure `README.md` index is present and current.
2. If only `docs/ops/decisions/` exists → recommend renaming to `docs/ops/adrs/` for tool/community alignment (ADR tooling expects `docs/ops/adrs/`).
...
```bash
npm i -g adr-log
adr-log -d docs/ops/adrs -i docs/ops/adrs/README.md
```
...
```bash
adr generate toc > docs/ops/adrs/README.md
```
...
```markdown
- **Architecture Decision Records:** `docs/ops/adrs/` — see `docs/ops/adrs/README.md` for the indexed log; newest-first
- **Lightweight decisions** (if your team uses the split): `docs/ops/decisions/`
```

In the `Memory primitives` section:

```markdown
- **Before making architectural decisions**, consult the decision log at `docs/ops/adrs/README.md`. If your proposed change conflicts with an `Accepted` ADR, write a new ADR superseding it; don't silently override.
```

## Audit checks for the decision log

1. **`docs/ops/adrs/` exists** (or `docs/ops/architecture/decisions/` — accepted alias).
2. **An index file exists** at `docs/ops/adrs/README.md` listing all ADRs.
...
6. **If `docs/ops/decisions/` also exists**, AGENTS.md documents the split rule.
...
- **Two parallel logs that drift** — `docs/ops/adrs/` and `docs/ops/decisions/` both have ADRs about Postgres. Pick a rule and document it.
- **Decision log as a single Markdown file** — `docs/ops/decisions.md` with all decisions in one document. Editing one decision rewrites file history; no per-decision audit trail. Strongly recommended against.
- **Folder named `docs/ops/architecture-decisions/`** — works, but `docs/ops/adrs/` is what tooling expects. Prefer `docs/ops/adrs/`.
- **Numbering reset on folder split** — if you split into `docs/ops/adrs/` and `docs/ops/decisions/`, do NOT restart numbering at 0001 in the second folder. Keep the global sequence so cross-references work.
