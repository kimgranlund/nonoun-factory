...
> **Storage convention.** This skill recommends `docs/ops/postmortems/` (most common) or `docs/ops/incidents/` (also common). Avoid `docs/ops/traces/` — "traces" risks being read as observability traces (OpenTelemetry / Jaeger), which is a different concept.
...
`docs/ops/incidents/` is an accepted alias. `docs/ops/post-mortems/` (with hyphen) also works. Pick one and stick with it.
...
```markdown
- **Post-mortems:** `docs/ops/postmortems/` — newest-first; read when investigating recurring issues
```

In the `Memory primitives` section:

```markdown
- **When debugging a production issue**, search `docs/ops/postmortems/` for prior occurrences. Many "new" bugs are repeats of fixed-and-forgotten issues.
```

## Audit checks for post-mortems

1. **Folder exists** at `docs/ops/postmortems/` (or `docs/ops/incidents/` — accepted alias).
...
5. **`docs/ops/postmortems/` is referenced from AGENTS.md**.

The audit should NOT flag a missing `docs/ops/postmortems/` folder for projects that haven't had production incidents — heuristic: search commit log for `revert`, `incident`, `outage`, `hotfix` to estimate whether the project should have post-mortems.
...
**Mapping to `repo-ops` artifacts.** When a post-mortem identifies a structural fix (new trip-wire, new convention, new generator), record the finding-to-fix pairing in `docs/ops/findings/INDEX.md` under `## Graduations`. The post-mortem is the narrative; the graduations table is the structured index. See [`../recipes/findings-index-readout.md`](../recipes/findings-index-readout.md).

## Common anti-patterns

- **Folder named `docs/ops/traces/`** — confused with observability traces.
