...
|  | `docs/ops/PLAN.md` | `docs/ops/ROADMAP.md` |
| --- | --- | --- |
...
See `docs/ops/archive/plan-2026-W16.md`.
```
...
- **Archive completed work weekly**: copy the "Done" section to `docs/ops/archive/plan-YYYY-Www.md` and clear it from `PLAN.md`. The archive is a free changelog.
...
The pattern: a CI job (or pre-commit hook) regenerates `docs/ops/PLAN.md` from the tracker on a cron schedule, commits the diff. Now the agent reads a current plan without an API token.
...
```markdown
- **Active plan:** `docs/ops/PLAN.md` — what's in flight this week
- **Roadmap:** `docs/ops/ROADMAP.md` — quarterly horizon
```

In the `Memory primitives` section:

```markdown
- **When picking up work**, start at `docs/ops/PLAN.md`. Items there are scoped for the current iteration.
- **For longer-horizon questions** ("are we going to support X?"), check `docs/ops/ROADMAP.md` before proposing new work.
```
...
5. **`docs/ops/PLAN.md` and `docs/ops/ROADMAP.md` are referenced from AGENTS.md.**
6. **Archive folder exists** if the team is using weekly archival (`docs/ops/archive/plan-YYYY-Www.md`).
