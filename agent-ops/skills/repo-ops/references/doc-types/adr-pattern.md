...
So the canonical "decision log" is **the index of all ADRs** in `docs/ops/adrs/`, not a separate folder. Generate it from filenames + status fields, or maintain `docs/ops/adrs/README.md` as a hand-written index.

Some teams _additionally_ maintain a lighter `docs/ops/decisions/` folder for non-architectural decisions (tooling picks, library swaps, day-to-day commitments). That's a **team convention, not industry standard** — but it's a useful split when ADRs feel too heavyweight for routine choices. The audit should detect both shapes and not flag either as wrong; just note which convention is in play.
...
- **For new projects, no tool is required.** `mkdir -p docs/ops/adrs && cp ~/.adr-templates/madr-minimal.md docs/ops/adrs/0001-record-architecture-decisions.md` is enough. Tooling adds friction; many teams now skip it.
...
```markdown
- **Architecture Decision Records:** `docs/ops/adrs/` — newest-first; read before making architectural changes
```

In the `Memory primitives` section:

```markdown
- **Before making architectural decisions**, read `docs/ops/adrs/` newest-first. If your proposed change conflicts with an `Accepted` ADR, write a new ADR superseding it; don't silently override.
```

## Common anti-patterns

- **No `docs/ops/adrs/` folder at all** — decisions live in random PR descriptions, Slack, code comments. Memory fragmentation.
...
1. **Folder exists** at `docs/ops/adrs/` (or close — `docs/ops/architecture/decisions/`, `adrs/`, etc.).
...
5. **`docs/ops/adrs/` is referenced from AGENTS.md**.
