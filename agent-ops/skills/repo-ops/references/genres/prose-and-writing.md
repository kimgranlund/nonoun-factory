...
The dotfile prefix is _more_ useful for prose than for code: drafts live at the repo root or in `chapters/` / `posts/`, and `docs/ops/` is structurally clearly the "out of band" memory layer that humans curate but don't read every day.
...
- `../audit-patterns/orphan-detection.md` — orphan files in `docs/ops/` not reachable from STYLE.md
