...
ARCHITECTURE.md  (~500 lines: system overview, component map,
   ...
   ↓ links to
docs/ops/adrs/0001-*.md  (each ADR ~100 lines: one decision in depth)
docs/ops/postmortems/2026-04-*.md  (each ~150 lines: one incident)
docs/ops/runbooks/*.md  (each ~100 lines: one procedure)
```
...
| --- | --- | --- | --- |
| CLAUDE.md | 200 | 2-3K | Every session (Claude Code, native) |
| AGENTS.md | 15 (thin pointer or symlink) | 0-200 | Opt-in (cross-tool pointer) |
| README.md | 200-300 | 3-5K | Sometimes (human-facing) |
| ARCHITECTURE.md | 500 | 6-8K | When agent needs system overview |
| docs/ops/PLAN.md | 200 | 2-3K | When agent needs current priorities |
| `docs/ops/adrs/<one>.md` | 100-200 | 1-3K each | When agent makes architectural decisions |
| `docs/ops/postmortems/<one>.md` | 100-200 | 1-3K each | When agent debugs production issues |
| `docs/ops/runbooks/<one>.md` | 100-200 | 1-3K each | When agent runs operational procedures |
...
3. **A `docs/ops/` file > 500 lines that isn't in `docs/ops/adrs/`, `docs/ops/postmortems/`, or `docs/ops/architecture/`** — likely a place where one big doc should be a folder of smaller ones.
...
- **Long ARCHITECTURE.md** — within reason; if it gets >1500 lines, split into `docs/ops/architecture/`.
...
3. **Strip TODOs / WIP notes.** Move to `docs/ops/PLAN.md` and link.
...
6. **Replace inline detail with a link.** "Our deployment uses Cloud Run with the following 14 steps..." → "Deployment: see `docs/ops/runbooks/deploy.md`".
...
**Pattern A — extract to a linked file.** A "Conventions" section growing past 30 lines becomes `docs/ops/CONVENTIONS.md` linked from AGENTS.md. The link is permanent; the file can grow.
