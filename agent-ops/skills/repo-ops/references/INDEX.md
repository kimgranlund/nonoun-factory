...
_Authoritative manifest of reference files. v1.7.0 — released 2026-05-02 (skill-stewardship as co-equal artifact class; harvest+import flow operations; +4 audit-patterns; v1.6 added findings-index readout + postmortem trigger rules; v1.5 introduced `docs/ops/` layout + prose-and-writing genre axis)._
...
| — | **Reliability dial** (cross-cutting) | `guidance/reliability-dial.md` (configures every trip-wire's threshold via `.claude/repo-ops.toml`) |
...
| Status | Path | Purpose |
| --- | --- | --- |
| ✅ | `standards/claude-md-convention.md` | CLAUDE.md as the canonical entry (Claude-native; Claude Code reads it natively) |
| ✅ | `standards/agents-md-spec.md` | AGENTS.md as the opt-in cross-tool standard (AAIF/Linux Foundation, Aug 2025, 60K+ projects) |
...
| --- | --- | --- | --- |
| ✅ | `audit-patterns/archive-link-sweep.md` | Depth-aware path rewrites after cross-tree doc moves (e.g. docs/X.md → docs/ops/archive/Y.md); per-file `(old, new)` tuples in Python | 1, 3 |
| ✅ | `audit-patterns/audit-history-ledger.md` | Persistent JSON record per audit run (`docs/ops/audit-history/`); SOC2-friendly | 5 |
...
| ✅ | `audit-patterns/entry-file-coverage.md` | Entry files exist, well-formed, point at `docs/ops/` | 1 |
...
| ✅ | `audit-patterns/memory-fragmentation.md` | Scattered ADRs, missing memory-primitives section in CLAUDE.md | 5 |
| ✅ | `audit-patterns/orphan-detection.md` | Files in `docs/ops/` or `docs/` not reachable from any entry (transitive closure) | 1 |
...
| ✅ | `guidance/reliability-dial.md` | `.claude/repo-ops.toml` strictness (lax / normal / strict) + git-sync (shared / local-only); per-trip-wire threshold routing | cross-cutting |
...
| ✅ | `recipes/audit-existing-repo.md` | Full audit pass with 7-step procedure + `docs/ops/` migration recipe | All |
...
| ✅ | `recipes/findings-index-readout.md` | Close the read side of the audit ledger — `docs/ops/findings/INDEX.md` aggregates all `findings[]` across `audit-history/*.json`; status taxonomy with auto-promotion to CLOSED-LATER; hand-curated `## Graduations` section | 5 |
| ✅ | `recipes/greenfield-setup.md` | Day-one setup with `docs/ops/` layout that ships all 5 promises from first commit | All |
| ✅ | `recipes/harvest-repo-brain.md` | **Flow operation**: lift findings from `docs/ops/` + `docs/conventions/` into agent memory at `~/.claude/projects/<repo>/memory/` and (optionally) emit a portable bundle for cross-repo ingestion | Cross-cutting |
| ✅ | `recipes/import-repo-brain-harvest.md` | **Flow operation**: inverse of harvest. Consumes a portable bundle into this repo's `docs/ops/` then runs the harvest pipeline so merged content propagates to memory + harness | Cross-cutting |
...
`CLAUDE.md` is the canonical entry; config lives under `.claude/`; the planning/decision docs live under `docs/ops/`:

```text
repo-root/
├── CLAUDE.md             # entry file (canonical, Claude-native — read natively)
├── AGENTS.md             # opt-in: symlink → CLAUDE.md OR thin pointer (cross-tool)
├── README.md
├── CONTRIBUTING.md
├── SECURITY.md
...
│   ├── hooks/            # PostToolUse / pre-commit hooks
│   ├── skills/           # procedural memory (skills)
│   └── repo-ops.toml     # strictness dial + tool stack (was docs/ops/config.toml)
├── docs/ops/
│   ├── PLAN.md           # active plan (optional)
│   ├── ROADMAP.md        # forward plan (optional)
│   ├── adrs/             # numbered ADRs
...
│   ├── changesets/       # CLAUDE.md changesets staging (multi-contributor)
