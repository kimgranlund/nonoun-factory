...
| --- | --- | --- | --- |
| 1 | **No `docs/ops/adrs/` folder despite architectural-shape commits** | `git log --grep='migrate\|switch from\|adopt'` returns 20 hits; `docs/ops/adrs/` doesn't exist | High |
| 2 | **No `docs/ops/postmortems/` despite incident-shape commits** | `git log --grep='hotfix\|revert\|incident\|outage'` returns matches; `docs/ops/postmortems/` empty | High |
| 3 | **Decision/RFC/ADR text scattered outside the canonical folders** | `grep -ri 'decision\|ADR\|RFC' docs/` finds matches in random `docs/notes-*.md` files, not `docs/ops/adrs/` | Medium |
...
Signal 4 is the most damaging. A repo can have an exemplary `docs/ops/adrs/` folder with 40 decisions, and if AGENTS.md doesn't tell the agent to _read it_, the artifacts are dead weight on disk.
...
if [ ! -d docs/ops/adrs ] && [ ! -d docs/adrs ] && [ ! -d docs/architecture/decisions ] && [ ! -d adrs ]; then
    [ "$arch" -gt 5 ] && { echo "FRAGMENT-NO-ADR-FOLDER: $arch arch-shape commits in 2y; no docs/ops/adrs/."; fail=1; }
else
    n=$(find docs/ops/adrs docs/adrs docs/architecture/decisions adrs -name '*.md' \
...
inc=$(git log --grep='hotfix\|revert\|incident\|outage\|rollback\|emergency' \
      --oneline 2>/dev/null | wc -l | tr -d ' ')
if [ ! -d docs/ops/postmortems ] && [ ! -d docs/postmortems ] && [ ! -d docs/incidents ] && [ ! -d docs/post-mortems ]; then
...
canonical_re='\docs/ops/(adrs|postmortems|architecture)/|docs/(adrs|architecture/decisions|postmortems|incidents|post-mortems)/'
find docs/ops docs -type f -name '*.md' -not -path 'docs/ops/archive/*' -not -path 'docs/archive/*' | while read -r f; do
...
### Signal 1 — no `docs/ops/adrs/` despite architectural commits
...
git mv docs/notes-on-postgres-decision.md docs/ops/adrs/0014-use-postgres-not-mysql.md
...
- **Before architectural changes**, read `docs/ops/adrs/` newest-first. If your
  proposed change conflicts with an `Accepted` ADR, write a new ADR
  superseding it; don't silently override.

- **When debugging a production issue**, search `docs/ops/postmortems/` for
...
- **When a runbook is needed for a recurring operation**, file it in
  `docs/ops/runbooks/` rather than embedding it in AGENTS.md.
```
...
The fix is cultural, not mechanical: the team agrees that "decision in a PR description" → "ADR in `docs/ops/adrs/`" before merge. The PR template change is in `../recipes/continuous-learning-loop.md` (Flow 2).
...
| `docs/ops/adrs/` missing AND repo has architectural-shape commits | High | Decisions are evaporating into PR descriptions |
| `docs/ops/postmortems/` missing AND repo has incident-shape commits | High | Lessons aren't compounding |
...
- **Detecting stale ADRs** — ADRs are immutable; "stale" doesn't apply. An ADR replaced by a newer one is `Superseded`, not `stale`. The audit explicitly excludes `docs/ops/adrs/` from `stale-content.md`'s heuristic 1.
...
- **Runbooks** — `docs/ops/runbooks/` is mentioned in the Memory primitives template but isn't its own audit category yet (could be added in v0.3.0).
