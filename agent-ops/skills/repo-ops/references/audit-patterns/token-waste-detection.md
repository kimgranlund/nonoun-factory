...
**The check:** `find docs/ops docs -name '*.md' -not -path 'docs/ops/adrs/*' -not -path 'docs/ops/postmortems/*' -not -path 'docs/ops/architecture/*'` and any file with `wc -l > 500`.
...
- `docs/ops/adrs/<one>.md` can be any length — one ADR captures one decision in full.
- `docs/ops/postmortems/<one>.md` can be any length — incidents need depth.
- `docs/ops/architecture/<one>.md` may be long; consider splitting into multiple files in the folder if so.
- `CHANGELOG.md` is append-only; it grows without bound.

**Auto-detection:**

```bash
find docs/ops docs -name '*.md' -type f \
...
for f in docs/ops/**/*.md docs/**/*.md *.md; do
    [ -f "$f" ] || continue
