...
threshold_days=180
now=$(date +%s)
find docs/ops docs -type f -name '*.md' \
    -not -path 'docs/ops/archive/*' -not -path 'docs/ops/adrs/*' -not -path 'docs/ops/postmortems/*' \
    ...
    | while read -r f; do
        ...
        grep -oE '`[A-Z][a-zA-Z0-9_]{3,}`' "$f" \
            | tr -d '`' | sort -u | while read -r sym; do
                git grep -q -wF "$sym" -- ':!*.md' ':!docs/ops/**' ':!docs/**' 2>/dev/null \
...
| Heuristic 2 — symbol referenced in `docs/ops/**` or `docs/**` no longer exists | Medium | Lower visibility but still wrong |
...
For semantic rot the heuristics miss ("the doc says we use Postgres but actually we migrated to PlanetScale six months ago" — no symbol-name overlap, no command-block giveaway), the answer is **LLM-on-diff** per `staleness-tooling.md`. On every PR, an LLM compares the code diff against `AGENTS.md` and `docs/ops/**` + `docs/**` and flags doc sections that should have changed. It's the most expensive layer (API calls per PR) and it's gated on `vars.LLM_DOC_DRIFT_ENABLED` per `../recipes/self-healing-hooks.md`.
