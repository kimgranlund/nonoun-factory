...
| 4 | ADR has `Status:` field | `docs/ops/adrs/*.md` | High |
| 5 | Postmortem has `severity:` and `duration:` | `docs/ops/postmortems/*.md` | High |
| 6 | Release-scoped working note has `created:` + `last_edited:` | `docs/ops/notes/{version}-*.md` | High |
...
for f in AGENTS.md CLAUDE.md README.md CHANGELOG.md \
         $(find docs/ops docs -name '*.md' -not -path 'docs/ops/archive/*' -not -path 'docs/archive/*' 2>/dev/null); do
  [ -f "$f" ] && [ ! -L "$f" ] || continue
...
For reference material in `docs/ops/` (not narrative READMEs in `docs/`), missing frontmatter is a smell. The repo-ops convention mirrors this skills library: `date`, `coverage`, `peers`, `primary_sources`, `status`.

### Detection

```bash
# A doc has frontmatter if line 1 is exactly `---`.
for f in $(find docs/ops docs -name '*.md' -not -path 'docs/ops/archive/*' -not -path 'docs/archive/*' 2>/dev/null); do
  [ -f "$f" ] || continue
  if [ "$(head -1 "$f")" != "---" ]; then
...
for f in $(find docs/ops/adrs docs/adrs docs/adr docs/decisions -name '*.md' 2>/dev/null); do
  [ -f "$f" ] || continue
...
```bash
for f in $(find docs/ops/postmortems docs/postmortems docs/incidents -name '*.md' 2>/dev/null); do
  [ -f "$f" ] || continue
...
`docs/ops/notes/` accumulates the narrative artifacts a release/build cycle produces — release notes, plans, punchlists, peer hand-offs — the markdown complement to the structured `audit-history/*.json`. Without dated frontmatter these rot silently (the Check-1 premise applied to the busiest churn surface in the brain).

**The convention** (transferable; first instantiated in chat-ui, adia-ui-release v1.10.0):

- **Naming** — release-scoped notes are `{release-version}-{topic}.md` (e.g. `0.6.49-release-notes.md`). Version-first sorts the directory by cut and makes `ls docs/ops/notes/ | grep <version>` the natural query. Non-release working notes keep `{topic}-YYYY-MM-DD.md`; pre-convention files are grandfathered.
...
```bash
for f in $(find docs/ops/notes -maxdepth 1 -name '*.md' 2>/dev/null); do
  base="$(basename "$f")"
...
- **ADR-NO-STATUS — `docs/ops/adrs/0007-pick-postgres.md`** (severity: high)
  - **Recommendation:** add `Status: accepted` (or `superseded by 0014`). See `../doc-types/adr-pattern.md`.

- **POSTMORTEM-NO-SEVERITY — `docs/ops/postmortems/2026-03-15-checkout-outage.md`** (severity: high)
...
| Reference doc in `docs/ops/` without YAML frontmatter | Medium | Convention drift; advisory for narrative docs |
