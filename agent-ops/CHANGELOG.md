...
- **`repo-ops` is now Claude-native by default; `AGENTS.md` / `.agents/` is an opt-in cross-tool mode.** The skill's canonical layout flips: **`CLAUDE.md`** is the canonical entry (Claude Code reads it natively — no symlink dance), **`.claude/`** is the config home (settings, hooks, `.claude/skills/` procedural memory, and the strictness dial `.claude/repo-ops.toml`, was `.agents/brain/config.toml`), and **`docs/ops/`** is the one home for the planning/decision docs (goals · roadmap · ADRs · issues · postmortems · runbooks · audit-history). `README.md` + `CHANGELOG.md` stay at the project root. The prior **`AGENTS.md`-canonical + `.agents/brain/` layout is preserved as an explicit opt-in mode** (`SKILL.md` § *Opt-in*) — used only when the repo serves cross-tool agents (Codex/Cursor/Devin/Copilot) or asks for it; in that mode `CLAUDE.md` stays canonical and `AGENTS.md` is a thin pointer/symlink to it (`ln -s CLAUDE.md AGENTS.md`). The flip spans `SKILL.md` (stance, invariants, layout, quick-reference, the five-promise trip-wires, §SelfAudit) + the load-bearing references (`claude-md-convention`, `agents-md-spec`, `cross-tool-matrix`, `entry-file-coverage`, `greenfield-setup`, `audit-existing-repo`, `cold-start-harvest`, `redundancy-detection`, `pointer-validation`, `coverage-gaps`, `reliability-dial`, `context-budget`, `self-healing-hooks`, `INDEX`, …) — a clean path rename (`.agents/brain/` → `docs/ops/`, `.agents/skills/` → `.claude/skills/`) plus a polarity flip of every entry-file assertion. Legacy `docs/{adrs,…}/` and the opt-in `.agents/brain/` are still recognized by the audit and read as equivalents.
- **The mechanism + gates follow the default.** The advisory **`doc-hygiene`** hook now treats `CLAUDE.md` as canonical (the DRIFT-RISK smell fires on a fat `AGENTS.md` that doesn't point at `CLAUDE.md`, not the reverse) and recognizes `docs/ops/{adr,adrs,postmortems,runbooks,decisions}/` as memory dirs. **`audit-history.py`** resolves the ledger at `docs/ops/audit-history/` first, falling back to `.agents/brain/audit-history/` (reads scan both; writes/index prefer the Claude-native dir). The **`repo-memory` MCP**'s `read_audit_ledger` does the same. The drift-check template script is renamed `check-agents-claude-drift.sh` → `check-entry-pointer-drift.sh`. All selftests green (`doc-hygiene`, `audit-history.py`, `repo-memory-mcp.py`). No `plugin.json` `stateNamespace` change — `.agents/brain` remains a valid (opt-in) state namespace under `.agents/`. plugin.json 0.1.18 → 0.1.19.

## [0.1.18] — 2026-06-17

### Added

- **A 13th council critic — `critic-jake-v` ("Jake V."), in the Architecture & Utility sub-council** — the lens distilled from Jake Van Clief's published work: the **Interpretable Context Methodology / Model Workspace Protocol** (folder-structure-as-agentic-architecture, arXiv 2603.16021) and the **Ethics Engine** / "Mind the Moral Echo" psychometric-audit work on LLM self-presentation bias (arXiv 2510.11742). One stance — **"Nothing Hidden, Give It Room"**, three pillars: (1) **organize the agent's whole world as a legible, folder-based filesystem** — workflow stages AND its skills/tools/reference-material bundled as navigable files ("plan with folders before building code") — rather than coordination buried in framework code or a scattered/hardcoded capability surface; (2) **give the model room to think and act** — focused, layered context (not a monolithic dump) and freedom to navigate the structure and decide (banks, not a cage; every output an editable surface, mixed-initiative) rather than boxing a capable model into rigid steps; (3) the success signal must be an **independent check of actual output**, never the agent — or a model of its own family — grading itself ("fake good" self-presentation bias). Carries the meta-warning that this council is itself Claude-family models grading agentic systems — a self-graded scoreboard — and folds that check into §SelfAudit.
- **Rubric dimension `A7` — "Interpretability & measurement integrity"** in `rubric-agentic-architecture.md` (now **7 dimensions**), with a 1–5 matrix, a 9th first-principle ("organize the world as legible folders, give the model room — and keep the scoreboard independent"), anti-pattern `AP-A7`, and two `§Hard Tests` (visible-coordination, fake-good). The full council is now **13 critics**; the agentic-ux skill's native council is **9**. Obscured per D-4 — the real attribution + sources live in the git-ignored `agents/.name-map.md`; `check-sourcing` green. plugin.json 0.1.17 → 0.1.18.
